...
class AboutDialog(QDialog):
    ...
    def make_button_box(self) -> QLayout:
        def ok():
            but = QPushButton('Close')
            qconnect(but.clicked, self.accept)
            but.setFixedHeight(BUTTON_HEIGHT)
            return but

        def dev_log():
            but = QPushButton("Open Dev-log")
            qconnect(but.clicked, lambda: showText(LogDebug().get_contents()))
            but.setFixedHeight(BUTTON_HEIGHT)
            return but

        but_box = QHBoxLayout()
        but_box.addWidget(ok())
        but_box.addWidget(dev_log())
        but_box.addStretch()
...
def menu_root_entry() -> QMenu:
    if not hasattr(mw.form, 'subsearch_root_menu'):
        mw.form.subsearch_root_menu = QMenu(f'&{ADDON_SERIES}', mw)
        mw.form.menubar.insertMenu(mw.form.menuHelp.menuAction(), mw.form.subsearch_root_menu)
        mw.form.subsearch_root_menu.addAction(create_about_action(mw.form.subsearch_root_menu))
        mw.form.subsearch_root_menu.addSeparator()
        print(f"{addon_folder_name()} inserted a new menu.")
    return mw.form.subsearch_root_menu
