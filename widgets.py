...
class SearchResultLabel(QLabel):
    ...
    def set_count(self, found: int = 0, npp: int = 100, displayed: int = 0, page: int = 1, custom_text: str = ''):
        if custom_text:
            ...
            self.setText(f'Displaying {displayed}/{found} Notes, Page {page}/{ceil(found/(ceil(displayed/npp)*npp))}')
...
class StatusBar(QHBoxLayout):
    ...
    def _add_fails_label(self):
        self._fails_label = QLabel()
        self._fails_label.setStyleSheet('QLabel { color: #ff4d4d; }')
        self.addWidget(self._fails_label)

    def hide(self):
        self._dupes_label.hide()
        self._success_label.hide()
        self._fails_label.hide()

    def set_status(self, successes: int = 0, dupes: int = 0, fails: int = 0, custom_text: str = ''):
        if successes:
        ...
        if fails:
            self._fails_label.setText(f'{fails} notes failed and were skipped.')
            self._fails_label.show()
        elif not fails and custom_text:
            self._fails_label.setText(custom_text)
            self._fails_label.show()
        else:
            self._fails_label.hide()
...
class NoteList(QWidget):
    ...
    def selected_notes(self) -> Sequence[dict]:
    ...
    def set_notes(self, notes: list, hide_fields: list[str], previewer: bool = True):
        ...
        for note in notes:
            ...
            item.setText(' | '.join(field_content
                for field_name, field_content in note.items()
                if not (is_hidden(field_name) or field_name == 'needed_media') and field_content.strip()
            ))
